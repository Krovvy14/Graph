...
def main():
    ...
    if args.write_file:
        ...
        try:
            ...
            server_process = Process(target=server.server, args=(csv_dir, 8000))    
            webapp_process = Process(target=server.server, args=(web_dir, 8080))
        ...
        try:
            ...
            server_process = Process(target=server.server, args=(csv_dir, 8000))
            web_process = Process(target=server.server, args=(web_dir, 8080))
            server_process.start()
            web_process.start()
